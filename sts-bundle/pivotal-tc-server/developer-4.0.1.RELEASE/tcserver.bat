...
rem ---------------------------------------------------------------------------
rem version: 4.0.1.RELEASE
rem build date: 20180424162821
...
"%JAVA_HOME%\bin\java" -Djava.util.logging.config.file="%SCRIPT_DIR%\conf\commons-logging.properties" -Dtcs.terminal.columns="%COLUMNS%" %JAVA_OPTS% -Dtcs.version=4.0.1.RELEASE -Dtcs.edition=@edition@ -Druntimes.directory="%RUNTIME_DIR%" -Ddefault.instances.directory="%INSTANCE_DIR%" -Dinstallation.directory="%INSTALLATION_DIR%" -classpath %CLASSPATH% com.springsource.tcruntime.instance.TcServer %*
SET _RET=%ERRORLEVEL%
IF %_RET% EQU 1 (
    IF "%1"=="" (
        pause
